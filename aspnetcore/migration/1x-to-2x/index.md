...
O exemplo anterior carrega o `Configuration` membro com definições de configuração de *appsettings.json* , bem como quaisquer *appSettings. \<EnvironmentName\> arquivo JSON* correspondente à `IHostingEnvironment.EnvironmentName` propriedade. Estes arquivos estão localizados no mesmo caminho que *Startup.cs* .
