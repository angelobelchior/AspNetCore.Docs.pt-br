...
Conforme explicado na documentação sobre [diferenças de versões anteriores do roteamento](xref:fundamentals/routing#differences-from-earlier-versions-of-routing), há algumas diferenças na geração de link (usando `Url.Link` e APIs semelhantes, por exemplo). Estão incluídos:
