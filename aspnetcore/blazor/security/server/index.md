---
title: 'Proteger ASP.NET Core :::no-loc(Blazor Server)::: aplicativos'
author: guardrex
description: 'Saiba como proteger aplicativos :::no-loc(Blazor Server)::: como ASP.NET Core aplicativos.'
...
---
# <a name="secure-aspnet-core-no-locblazor-server-apps"></a><span data-ttu-id="aedf5-103">Proteger ASP.NET Core :::no-loc(Blazor Server)::: aplicativos</span><span class="sxs-lookup"><span data-stu-id="aedf5-103">Secure ASP.NET Core :::no-loc(Blazor Server)::: apps</span></span>
...
<span data-ttu-id="aedf5-105">:::no-loc(Blazor Server)::: os aplicativos são configurados para segurança da mesma maneira que ASP.NET Core aplicativos.</span><span class="sxs-lookup"><span data-stu-id="aedf5-105">:::no-loc(Blazor Server)::: apps are configured for security in the same manner as ASP.NET Core apps.</span></span> <span data-ttu-id="aedf5-106">Para obter mais informações, consulte os artigos em <xref:security/index> .</span><span class="sxs-lookup"><span data-stu-id="aedf5-106">For more information, see the articles under <xref:security/index>.</span></span> <span data-ttu-id="aedf5-107">Os tópicos desta visão geral se aplicam especificamente ao :::no-loc(Blazor Server)::: .</span><span class="sxs-lookup"><span data-stu-id="aedf5-107">Topics under this overview apply specifically to :::no-loc(Blazor Server):::.</span></span>

## <a name="no-locblazor-server-project-template"></a><span data-ttu-id="aedf5-108">:::no-loc(Blazor Server)::: modelo de projeto</span><span class="sxs-lookup"><span data-stu-id="aedf5-108">:::no-loc(Blazor Server)::: project template</span></span>

<span data-ttu-id="aedf5-109">O :::no-loc(Blazor Server)::: modelo de projeto pode ser configurado para autenticação quando o projeto é criado.</span><span class="sxs-lookup"><span data-stu-id="aedf5-109">The :::no-loc(Blazor Server)::: project template can be configured for authentication when the project is created.</span></span>
...
<span data-ttu-id="aedf5-111">Siga as diretrizes do Visual Studio no <xref:blazor/tooling> para criar um novo :::no-loc(Blazor Server)::: projeto com um mecanismo de autenticação.</span><span class="sxs-lookup"><span data-stu-id="aedf5-111">Follow the Visual Studio guidance in <xref:blazor/tooling> to create a new :::no-loc(Blazor Server)::: project with an authentication mechanism.</span></span>

<span data-ttu-id="aedf5-112">Depois de escolher o modelo de **:::no-loc(Blazor Server)::: aplicativo** na caixa de diálogo **criar um novo ASP.NET Core aplicativo Web** , selecione **alterar** em **autenticação** .</span><span class="sxs-lookup"><span data-stu-id="aedf5-112">After choosing the **:::no-loc(Blazor Server)::: App** template in the **Create a new ASP.NET Core Web Application** dialog, select **Change** under **Authentication** .</span></span>
...
* <span data-ttu-id="aedf5-115">**Contas de usuário individuais** : as contas de usuário podem ser armazenadas:</span><span class="sxs-lookup"><span data-stu-id="aedf5-115">**Individual User Accounts** : User accounts can be stored:</span></span>
  * <span data-ttu-id="aedf5-116">Dentro do aplicativo usando o sistema do ASP.NET Core [:::no-loc(Identity):::](xref:security/authentication/identity) .</span><span class="sxs-lookup"><span data-stu-id="aedf5-116">Within the app using ASP.NET Core's [:::no-loc(Identity):::](xref:security/authentication/identity) system.</span></span>
...
<span data-ttu-id="aedf5-121">Siga as diretrizes de Visual Studio Code no <xref:blazor/tooling> para criar um novo :::no-loc(Blazor Server)::: projeto com um mecanismo de autenticação:</span><span class="sxs-lookup"><span data-stu-id="aedf5-121">Follow the Visual Studio Code guidance in <xref:blazor/tooling> to create a new :::no-loc(Blazor Server)::: project with an authentication mechanism:</span></span>
...
| `Individual`             | <span data-ttu-id="aedf5-127">Usuários armazenados no aplicativo com :::no-loc(ASP.NET Core Identity):::</span><span class="sxs-lookup"><span data-stu-id="aedf5-127">Users stored in the app with :::no-loc(ASP.NET Core Identity):::</span></span> |
...
1. <span data-ttu-id="aedf5-138">Na etapa **configurar seu novo :::no-loc(Blazor Server)::: aplicativo** , selecione **autenticação individual (no aplicativo)** na lista suspensa **autenticação** .</span><span class="sxs-lookup"><span data-stu-id="aedf5-138">On the **Configure your new :::no-loc(Blazor Server)::: App** step, select **Individual Authentication (in-app)** from the **Authentication** drop down.</span></span>

1. <span data-ttu-id="aedf5-139">O aplicativo é criado para usuários individuais armazenados no aplicativo com :::no-loc(ASP.NET Core Identity)::: .</span><span class="sxs-lookup"><span data-stu-id="aedf5-139">The app is created for individual users stored in the app with :::no-loc(ASP.NET Core Identity):::.</span></span>
...
<span data-ttu-id="aedf5-141">Crie um novo :::no-loc(Blazor Server)::: projeto com um mecanismo de autenticação usando o seguinte comando em um shell de comando:</span><span class="sxs-lookup"><span data-stu-id="aedf5-141">Create a new :::no-loc(Blazor Server)::: project with an authentication mechanism using the following command in a command shell:</span></span>
...
| `Individual`             | <span data-ttu-id="aedf5-147">Usuários armazenados no aplicativo com :::no-loc(ASP.NET Core Identity):::</span><span class="sxs-lookup"><span data-stu-id="aedf5-147">Users stored in the app with :::no-loc(ASP.NET Core Identity):::</span></span> |
...
* <span data-ttu-id="aedf5-157">Execute o comando de ajuda para o :::no-loc(Blazor Server)::: modelo ( `blazorserver` ) em um shell de comando:</span><span class="sxs-lookup"><span data-stu-id="aedf5-157">Execute the help command for the :::no-loc(Blazor Server)::: template (`blazorserver`) in a command shell:</span></span>
...
## <a name="scaffold-no-locidentity"></a><span data-ttu-id="aedf5-158">Scaffold :::no-loc(Identity):::</span><span class="sxs-lookup"><span data-stu-id="aedf5-158">Scaffold :::no-loc(Identity):::</span></span>

<span data-ttu-id="aedf5-159">Scaffold :::no-loc(Identity)::: em um :::no-loc(Blazor Server)::: projeto:</span><span class="sxs-lookup"><span data-stu-id="aedf5-159">Scaffold :::no-loc(Identity)::: into a :::no-loc(Blazor Server)::: project:</span></span>
