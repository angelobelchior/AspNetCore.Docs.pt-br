...
---
# <a name="host-and-deploy-no-locblazor-server"></a><span data-ttu-id="3c421-103">Hospedar e implantar :::no-loc(Blazor Server):::</span><span class="sxs-lookup"><span data-stu-id="3c421-103">Host and deploy :::no-loc(Blazor Server):::</span></span>
...
<span data-ttu-id="3c421-106">os [ :::no-loc(Blazor Server)::: aplicativos](xref:blazor/hosting-models#blazor-server) podem aceitar [valores de configuração de host genéricos](xref:fundamentals/host/generic-host#host-configuration).</span><span class="sxs-lookup"><span data-stu-id="3c421-106">[:::no-loc(Blazor Server)::: apps](xref:blazor/hosting-models#blazor-server) can accept [Generic Host configuration values](xref:fundamentals/host/generic-host#host-configuration).</span></span>
...
<span data-ttu-id="3c421-113">Planeje uma implantação para fazer o melhor uso da infraestrutura disponível para um :::no-loc(Blazor Server)::: aplicativo.</span><span class="sxs-lookup"><span data-stu-id="3c421-113">Plan a deployment to make the best use of the available infrastructure for a :::no-loc(Blazor Server)::: app.</span></span> <span data-ttu-id="3c421-114">Consulte os seguintes recursos para abordar a :::no-loc(Blazor Server)::: escalabilidade do aplicativo:</span><span class="sxs-lookup"><span data-stu-id="3c421-114">See the following resources to address :::no-loc(Blazor Server)::: app scalability:</span></span>

* [<span data-ttu-id="3c421-115">Conceitos básicos de :::no-loc(Blazor Server)::: aplicativos</span><span class="sxs-lookup"><span data-stu-id="3c421-115">Fundamentals of :::no-loc(Blazor Server)::: apps</span></span>](xref:blazor/hosting-models#blazor-server)
...
<span data-ttu-id="3c421-121">Para obter orientação sobre a criação de :::no-loc(Blazor)::: aplicativos de servidor seguros e escalonáveis, consulte <xref:blazor/security/server/threat-mitigation> .</span><span class="sxs-lookup"><span data-stu-id="3c421-121">For guidance on building secure and scalable :::no-loc(Blazor)::: server apps, see <xref:blazor/security/server/threat-mitigation>.</span></span>
...
### <a name="no-locsignalr-configuration"></a><span data-ttu-id="3c421-125">:::no-loc(SignalR)::: configuração</span><span class="sxs-lookup"><span data-stu-id="3c421-125">:::no-loc(SignalR)::: configuration</span></span>

<span data-ttu-id="3c421-126">:::no-loc(Blazor Server)::: os aplicativos usam ASP.NET Core :::no-loc(SignalR)::: para se comunicar com o navegador.</span><span class="sxs-lookup"><span data-stu-id="3c421-126">:::no-loc(Blazor Server)::: apps use ASP.NET Core :::no-loc(SignalR)::: to communicate with the browser.</span></span> <span data-ttu-id="3c421-127">as [ :::no-loc(SignalR)::: condições de hospedagem e dimensionamento do](xref:signalr/publish-to-azure-web-app) se aplicam aos :::no-loc(Blazor Server)::: aplicativos.</span><span class="sxs-lookup"><span data-stu-id="3c421-127">[:::no-loc(SignalR):::'s hosting and scaling conditions](xref:signalr/publish-to-azure-web-app) apply to :::no-loc(Blazor Server)::: apps.</span></span>

<span data-ttu-id="3c421-128">:::no-loc(Blazor)::: funciona melhor ao usar o WebSocket como :::no-loc(SignalR)::: transporte devido à latência, confiabilidade e [segurança](xref:signalr/security)menores.</span><span class="sxs-lookup"><span data-stu-id="3c421-128">:::no-loc(Blazor)::: works best when using WebSockets as the :::no-loc(SignalR)::: transport due to lower latency, reliability, and [security](xref:signalr/security).</span></span> <span data-ttu-id="3c421-129">A sondagem longa é usada pelo :::no-loc(SignalR)::: quando o WebSockets não está disponível ou quando o aplicativo é explicitamente configurado para usar sondagem longa.</span><span class="sxs-lookup"><span data-stu-id="3c421-129">Long Polling is used by :::no-loc(SignalR)::: when WebSockets isn't available or when the app is explicitly configured to use Long Polling.</span></span> <span data-ttu-id="3c421-130">Ao implantar no serviço Azure App, configure o aplicativo para usar Websockets nas configurações de portal do Azure para o serviço.</span><span class="sxs-lookup"><span data-stu-id="3c421-130">When deploying to Azure App Service, configure the app to use WebSockets in the Azure portal settings for the service.</span></span> <span data-ttu-id="3c421-131">Para obter detalhes sobre como configurar o aplicativo para Azure App serviço, consulte as [ :::no-loc(SignalR)::: diretrizes de publicação](xref:signalr/publish-to-azure-web-app).</span><span class="sxs-lookup"><span data-stu-id="3c421-131">For details on configuring the app for Azure App Service, see the [:::no-loc(SignalR)::: publishing guidelines](xref:signalr/publish-to-azure-web-app).</span></span>

#### <a name="azure-no-locsignalr-service"></a><span data-ttu-id="3c421-132">Serviço do Azure :::no-loc(SignalR):::</span><span class="sxs-lookup"><span data-stu-id="3c421-132">Azure :::no-loc(SignalR)::: Service</span></span>

<span data-ttu-id="3c421-133">É recomendável usar [o :::no-loc(SignalR)::: serviço do Azure](xref:signalr/scale#azure-signalr-service) para :::no-loc(Blazor Server)::: aplicativos.</span><span class="sxs-lookup"><span data-stu-id="3c421-133">We recommend using the [Azure :::no-loc(SignalR)::: Service](xref:signalr/scale#azure-signalr-service) for :::no-loc(Blazor Server)::: apps.</span></span> <span data-ttu-id="3c421-134">O serviço permite escalar verticalmente um :::no-loc(Blazor Server)::: aplicativo para um grande número de conexões simultâneas :::no-loc(SignalR)::: .</span><span class="sxs-lookup"><span data-stu-id="3c421-134">The service allows for scaling up a :::no-loc(Blazor Server)::: app to a large number of concurrent :::no-loc(SignalR)::: connections.</span></span> <span data-ttu-id="3c421-135">Além disso, o :::no-loc(SignalR)::: alcance global do serviço e os data centers de alto desempenho auxiliam significativamente na redução da latência devido à geografia.</span><span class="sxs-lookup"><span data-stu-id="3c421-135">In addition, the :::no-loc(SignalR)::: service's global reach and high-performance data centers significantly aid in reducing latency due to geography.</span></span>
...
>
> <span data-ttu-id="3c421-138">É recomendável usar Websockets para :::no-loc(Blazor Server)::: aplicativos implantados no serviço Azure app.</span><span class="sxs-lookup"><span data-stu-id="3c421-138">We recommend using WebSockets for :::no-loc(Blazor Server)::: apps deployed to Azure App Service.</span></span> <span data-ttu-id="3c421-139">O [ :::no-loc(SignalR)::: serviço do Azure](xref:signalr/scale#azure-signalr-service) usa o WebSocket por padrão.</span><span class="sxs-lookup"><span data-stu-id="3c421-139">The [Azure :::no-loc(SignalR)::: Service](xref:signalr/scale#azure-signalr-service) uses WebSockets by default.</span></span> <span data-ttu-id="3c421-140">Se o aplicativo não usar o serviço do Azure :::no-loc(SignalR)::: , consulte <xref:signalr/publish-to-azure-web-app#configure-the-app-in-azure-app-service> .</span><span class="sxs-lookup"><span data-stu-id="3c421-140">If the app doesn't use the Azure :::no-loc(SignalR)::: Service, see <xref:signalr/publish-to-azure-web-app#configure-the-app-in-azure-app-service>.</span></span>
>
...
>
> * [<span data-ttu-id="3c421-142">O que é o serviço do Azure :::no-loc(SignalR)::: ?</span><span class="sxs-lookup"><span data-stu-id="3c421-142">What is Azure :::no-loc(SignalR)::: Service?</span></span>](/azure/azure-signalr/signalr-overview)
> * [<span data-ttu-id="3c421-143">Guia de desempenho do :::no-loc(SignalR)::: serviço do Azure</span><span class="sxs-lookup"><span data-stu-id="3c421-143">Performance guide for Azure :::no-loc(SignalR)::: Service</span></span>](/azure-signalr/signalr-concept-performance#performance-factors)
...
<span data-ttu-id="3c421-145">Para configurar um aplicativo para o serviço do Azure :::no-loc(SignalR)::: , o aplicativo deve dar suporte a *sessões adesivas* , em que os clientes são [redirecionados de volta para o mesmo servidor durante o pré-processamento](xref:blazor/hosting-models#connection-to-the-server).</span><span class="sxs-lookup"><span data-stu-id="3c421-145">To configure an app for the Azure :::no-loc(SignalR)::: Service, the app must support *sticky sessions* , where clients are [redirected back to the same server when prerendering](xref:blazor/hosting-models#connection-to-the-server).</span></span> <span data-ttu-id="3c421-146">A `ServerStickyMode` opção ou o valor de configuração é definido como `Required` .</span><span class="sxs-lookup"><span data-stu-id="3c421-146">The `ServerStickyMode` option or configuration value is set to `Required`.</span></span> <span data-ttu-id="3c421-147">Normalmente, um aplicativo cria a configuração usando **_uma_** das seguintes abordagens:</span><span class="sxs-lookup"><span data-stu-id="3c421-147">Typically, an app creates the configuration using **_ONE_** of the following approaches:</span></span>
   ...
   * <span data-ttu-id="3c421-149">Configuração (use **_uma_** das seguintes abordagens):</span><span class="sxs-lookup"><span data-stu-id="3c421-149">Configuration (use **_ONE_** of the following approaches):</span></span>
     ...
     * <span data-ttu-id="3c421-150">Em `:::no-loc(appsettings.json):::`:</span><span class="sxs-lookup"><span data-stu-id="3c421-150">In `:::no-loc(appsettings.json):::`:</span></span>

       ```json
       "Azure::::no-loc(SignalR)::::StickyServerMode": "Required"
       ```

     * <span data-ttu-id="3c421-151">As configurações do aplicativo de **configuração** do serviço de aplicativo  >  **Application settings** na portal do Azure ( **nome** : `Azure__:::no-loc(SignalR):::__StickyServerMode` , **valor** : `Required` ).</span><span class="sxs-lookup"><span data-stu-id="3c421-151">The app service's **Configuration** > **Application settings** in the Azure portal ( **Name** : `Azure__:::no-loc(SignalR):::__StickyServerMode`, **Value** : `Required`).</span></span> <span data-ttu-id="3c421-152">Essa abordagem será adotada para o aplicativo automaticamente se você [provisionar o :::no-loc(SignalR)::: serviço do Azure](#provision-the-azure-signalr-service).</span><span class="sxs-lookup"><span data-stu-id="3c421-152">This approach is adopted for the app automatically if you [provision the Azure :::no-loc(SignalR)::: Service](#provision-the-azure-signalr-service).</span></span>

### <a name="provision-the-azure-no-locsignalr-service"></a><span data-ttu-id="3c421-153">Provisionar o serviço do Azure :::no-loc(SignalR):::</span><span class="sxs-lookup"><span data-stu-id="3c421-153">Provision the Azure :::no-loc(SignalR)::: Service</span></span>

<span data-ttu-id="3c421-154">Para provisionar o serviço do Azure :::no-loc(SignalR)::: para um aplicativo no Visual Studio:</span><span class="sxs-lookup"><span data-stu-id="3c421-154">To provision the Azure :::no-loc(SignalR)::: Service for an app in Visual Studio:</span></span>

1. <span data-ttu-id="3c421-155">Crie um perfil de publicação de aplicativos do Azure no Visual Studio para o :::no-loc(Blazor Server)::: aplicativo.</span><span class="sxs-lookup"><span data-stu-id="3c421-155">Create an Azure Apps publish profile in Visual Studio for the :::no-loc(Blazor Server)::: app.</span></span>
1. <span data-ttu-id="3c421-156">Adicione a dependência de **:::no-loc(SignalR)::: serviço do Azure** ao perfil.</span><span class="sxs-lookup"><span data-stu-id="3c421-156">Add the **Azure :::no-loc(SignalR)::: Service** dependency to the profile.</span></span> <span data-ttu-id="3c421-157">Se a assinatura do Azure não tiver uma instância de serviço do Azure já existente :::no-loc(SignalR)::: para atribuir ao aplicativo, selecione **criar uma nova :::no-loc(SignalR)::: instância de serviço do Azure** para provisionar uma nova instância de serviço.</span><span class="sxs-lookup"><span data-stu-id="3c421-157">If the Azure subscription doesn't have a pre-existing Azure :::no-loc(SignalR)::: Service instance to assign to the app, select **Create a new Azure :::no-loc(SignalR)::: Service instance** to provision a new service instance.</span></span>
...
<span data-ttu-id="3c421-159">O provisionamento do serviço do Azure :::no-loc(SignalR)::: no Visual Studio habilita automaticamente as [ *sessões adesivas*](#configuration) e adiciona a :::no-loc(SignalR)::: cadeia de conexão à configuração do serviço de aplicativo.</span><span class="sxs-lookup"><span data-stu-id="3c421-159">Provisioning the Azure :::no-loc(SignalR)::: Service in Visual Studio automatically [enables *sticky sessions*](#configuration) and adds the :::no-loc(SignalR)::: connection string to the app service's configuration.</span></span>
...
<span data-ttu-id="3c421-165">Crie uma definição de entrada com as seguintes [anotações de kubernetes para sessões adesivas](https://kubernetes.github.io/ingress-nginx/examples/affinity/:::no-loc(cookie):::/):</span><span class="sxs-lookup"><span data-stu-id="3c421-165">Create an ingress definition with the following [Kubernetes annotations for sticky sessions](https://kubernetes.github.io/ingress-nginx/examples/affinity/:::no-loc(cookie):::/):</span></span>
...
<span data-ttu-id="3c421-167">Para :::no-loc(SignalR)::: que o WebSockets funcione corretamente, confirme se os `Upgrade` cabeçalhos e o proxy `Connection` estão definidos com os seguintes valores e se estão `$connection_upgrade` mapeados para:</span><span class="sxs-lookup"><span data-stu-id="3c421-167">For :::no-loc(SignalR)::: WebSockets to function properly, confirm that the proxy's `Upgrade` and `Connection` headers are set to the following values and that `$connection_upgrade` is mapped to either:</span></span>
...
<span data-ttu-id="3c421-174">Para hospedar um :::no-loc(Blazor)::: aplicativo por trás do Apache no Linux, configure `ProxyPass` para tráfego HTTP e WebSockets.</span><span class="sxs-lookup"><span data-stu-id="3c421-174">To host a :::no-loc(Blazor)::: app behind Apache on Linux, configure `ProxyPass` for HTTP and WebSockets traffic.</span></span>
