...
<span data-ttu-id="f4787-110">Este documento discute layouts para as duas abordagens diferentes para ASP.NET Core MVC: :::no-loc(Razor)::: páginas e controladores com exibições.</span><span class="sxs-lookup"><span data-stu-id="f4787-110">This document discusses layouts for the two different approaches to ASP.NET Core MVC: :::no-loc(Razor)::: Pages and controllers with views.</span></span> <span data-ttu-id="f4787-111">Para este tópico, as diferenças são mínimas:</span><span class="sxs-lookup"><span data-stu-id="f4787-111">For this topic, the differences are minimal:</span></span>

* <span data-ttu-id="f4787-112">:::no-loc(Razor)::: As páginas estão na pasta *páginas* .</span><span class="sxs-lookup"><span data-stu-id="f4787-112">:::no-loc(Razor)::: Pages are in the *Pages* folder.</span></span>
...
* <span data-ttu-id="f4787-123">:::no-loc(Razor)::: Páginas: *páginas/compartilhadas/_Layout. cshtml*</span><span class="sxs-lookup"><span data-stu-id="f4787-123">:::no-loc(Razor)::: Pages: *Pages/Shared/_Layout.cshtml*</span></span>
...
<span data-ttu-id="f4787-132">:::no-loc(Razor)::: as exibições têm uma `Layout` propriedade.</span><span class="sxs-lookup"><span data-stu-id="f4787-132">:::no-loc(Razor)::: views have a `Layout` property.</span></span> <span data-ttu-id="f4787-133">As exibições individuais especificam um layout com a configuração dessa propriedade:</span><span class="sxs-lookup"><span data-stu-id="f4787-133">Individual views specify a layout by setting this property:</span></span>

[!code-cshtml[](../../common/samples/WebApplication1/Views/_ViewStart.cshtml?highlight=2)]

<span data-ttu-id="f4787-134">O layout especificado pode usar um caminho completo (por exemplo, */Pages/Shared/_Layout.cshtml* ou */Views/Shared/_Layout.cshtml* ) ou um nome parcial (exemplo: `_Layout`).</span><span class="sxs-lookup"><span data-stu-id="f4787-134">The layout specified can use a full path (for example, */Pages/Shared/_Layout.cshtml* or */Views/Shared/_Layout.cshtml* ) or a partial name (example: `_Layout`).</span></span> <span data-ttu-id="f4787-135">Quando um nome parcial é fornecido, o :::no-loc(Razor)::: mecanismo de exibição pesquisa o arquivo de layout usando seu processo de descoberta padrão.</span><span class="sxs-lookup"><span data-stu-id="f4787-135">When a partial name is provided, the :::no-loc(Razor)::: view engine searches for the layout file using its standard discovery process.</span></span> <span data-ttu-id="f4787-136">A pasta em que o método do manipulador (ou controlador) existe é pesquisada primeiro, seguida pela pasta *Shared* .</span><span class="sxs-lookup"><span data-stu-id="f4787-136">The folder where the handler method (or controller) exists is searched first, followed by the *Shared* folder.</span></span> <span data-ttu-id="f4787-137">Esse processo de descoberta é idêntico àquele usado para descobrir [exibições parciais](xref:mvc/views/partial#partial-view-discovery).</span><span class="sxs-lookup"><span data-stu-id="f4787-137">This discovery process is identical to the process used to discover [partial views](xref:mvc/views/partial#partial-view-discovery).</span></span>
...
<span data-ttu-id="f4787-144">Se uma seção obrigatória não for encontrada, uma exceção será gerada.</span><span class="sxs-lookup"><span data-stu-id="f4787-144">If a required section isn't found, an exception is thrown.</span></span> <span data-ttu-id="f4787-145">Modos de exibição individuais especificam o conteúdo a ser processado em uma seção usando a `@section` :::no-loc(Razor)::: sintaxe.</span><span class="sxs-lookup"><span data-stu-id="f4787-145">Individual views specify the content to be rendered within a section using the `@section` :::no-loc(Razor)::: syntax.</span></span> <span data-ttu-id="f4787-146">Se uma página ou exibição definir uma seção, ela precisará ser renderizada (ou ocorrerá um erro).</span><span class="sxs-lookup"><span data-stu-id="f4787-146">If a page or view defines a section, it must be rendered (or an error will occur).</span></span>

<span data-ttu-id="f4787-147">Uma `@section` definição de exemplo no :::no-loc(Razor)::: modo de exibição páginas:</span><span class="sxs-lookup"><span data-stu-id="f4787-147">An example `@section` definition in :::no-loc(Razor)::: Pages view:</span></span>
...
<span data-ttu-id="f4787-151">A marcação anterior foi gerada por [scaffolding :::no-loc(Identity)::: ](xref:security/authentication/scaffold-identity).</span><span class="sxs-lookup"><span data-stu-id="f4787-151">The preceding markup was generated by [scaffolding :::no-loc(Identity):::](xref:security/authentication/scaffold-identity).</span></span>
...
<span data-ttu-id="f4787-155">Por padrão, o corpo e todas as seções de uma página de conteúdo precisam ser renderizados pela página de layout.</span><span class="sxs-lookup"><span data-stu-id="f4787-155">By default, the body and all sections in a content page must all be rendered by the layout page.</span></span> <span data-ttu-id="f4787-156">O :::no-loc(Razor)::: mecanismo de exibição impõe isso rastreando se o corpo e cada seção foram renderizados.</span><span class="sxs-lookup"><span data-stu-id="f4787-156">The :::no-loc(Razor)::: view engine enforces this by tracking whether the body and each section have been rendered.</span></span>
...
<span data-ttu-id="f4787-158">O corpo e cada seção em uma :::no-loc(Razor)::: página devem ser renderizados ou ignorados.</span><span class="sxs-lookup"><span data-stu-id="f4787-158">The body and every section in a :::no-loc(Razor)::: page must be either rendered or ignored.</span></span>
...
<span data-ttu-id="f4787-163">O arquivo não dá suporte a outros :::no-loc(Razor)::: recursos, como definições de funções e de seção.</span><span class="sxs-lookup"><span data-stu-id="f4787-163">The file doesn't support other :::no-loc(Razor)::: features, such as functions and section definitions.</span></span>
